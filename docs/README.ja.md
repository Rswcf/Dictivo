...
| そのまま貼り付けられる形に整えたい | 既定では通常の Message として出力し、`Settings -> Local Engine -> Processing toggles` で句読点、フィラー削除、大文字化を調整できます。 |
...
- macOS: `Dictivo.app` または `.dmg`
- Windows: 通常は `.exe` の現在ユーザー向けインストーラー、管理配布には `.msi`

Dictivo を開き、`Settings -> Local Engine` でローカルモデルをダウンロードまたはインポートします。
...
7. Dictivo がローカルで文字起こしし、最終テキストをコピーして現在のアプリへ貼り付けを試みます。

OS によって自動貼り付けがブロックされた場合でも、テキストはクリップボードにコピーされています。macOS では `Command+V`、Windows では `Ctrl+V` を押してください。
...
| ローカルモデルが見つからない | `Settings -> Local Engine` で `.bin` モデルをダウンロードまたはインポートします。 |
...
Dictivo はローカルファースト設計です。デスクトップアプリは音声入力のためにクラウド AI API を呼び出しません。
...
バックエンドは、ローカル session ID、provider 名、プライバシーモード、長さ、単語数などのメタデータだけを扱います。

## 対応言語

現在のアプリは次のローカル音声入力設定をサポートしています。
