...
| Windows installer/version smoke | Windows | Current app version | GitHub Actions | Automated | 2026-05-18: the latest `Build desktop apps` run installs the Windows NSIS artifact, verifies installed `Dictivo.exe` `ProductVersion`, and confirms the process stays running during launch smoke. This is not a replacement for `WIN-PARITY-001..020`. |
